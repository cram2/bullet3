...
class TestUserDataMethods(unittest.TestCase):
  ...
  def setUpClass(cls):
    TestUserDataMethods.server = bullet_client.BulletClient(
        connection_mode=pycram_bullet.SHARED_MEMORY_SERVER)
  ...
  def setUp(self):
    self.client = bullet_client.BulletClient(pycram_bullet.SHARED_MEMORY)
  ...
  def testMultipleClients(self):
    client1 = self.client
    client2 = bullet_client.BulletClient(pycram_bullet.SHARED_MEMORY)
