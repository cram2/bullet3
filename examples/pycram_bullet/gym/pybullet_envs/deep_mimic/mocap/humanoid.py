...
from pycram_bullet_utils.bullet_client import BulletClient
...
class Humanoid(object):
  ...
  def __init__(self, pycram_bullet_client, motion_data, baseShift):
    """Constructs a humanoid and reset it to the initial states.
    Args:
      pycram_bullet_client: The instance of BulletClient to manage different
        simulations.
    """
    self._baseShift = baseShift
    self._pycram_bullet_client = pycram_bullet_client

    self.kin_client = BulletClient(
        pycram_bullet_client.DIRECT
    )  # use SHARED_MEMORY for visual debugging, start a GUI physics server first
    self.kin_client.resetSimulation()
    self.kin_client.setAdditionalSearchPath(pycram_bullet_data.getDataPath())
    ...
    self._humanoid = self._pycram_bullet_client.loadURDF("humanoid/humanoid.urdf", [0, 0.9, 0],
                                                    globalScaling=0.25,
                                                    useFixedBase=False)
    ...
    self._pycram_bullet_client.resetBasePositionAndOrientation(self._humanoid, self._baseShift,
                                                          [0, 0, 0, 1])
    self._pycram_bullet_client.changeDynamics(self._humanoid, -1, linearDamping=0, angularDamping=0)
    for j in range(self._pycram_bullet_client.getNumJoints(self._humanoid)):
      ji = self._pycram_bullet_client.getJointInfo(self._humanoid, j)
      self._pycram_bullet_client.changeDynamics(self._humanoid, j, linearDamping=0, angularDamping=0)
      self._pycram_bullet_client.changeVisualShape(self._humanoid, j, rgbaColor=[1, 1, 1, 1])
      #print("joint[",j,"].type=",jointTypes[ji[2]])
      #print("joint[",j,"].name=",ji[1])

    self._initial_state = self._pycram_bullet_client.saveState()
    self._allowed_body_parts = [11, 14]
    self.Reset()

  def Reset(self):
    self._pycram_bullet_client.restoreState(self._initial_state)
    self.SetSimTime(0)
    pose = self.InitializePoseFromMotionData()
    self.ApplyPose(pose, True, True, self._humanoid, self._pycram_bullet_client)
  ...
  def RenderReference(self, t):
    ...
    pose = HumanoidPose()
    pose.Slerp(self._frameFraction, frameData, frameDataNext, self._pycram_bullet_client)
    self.ApplyPose(pose, True, True, self._humanoid, self._pycram_bullet_client)
  ...
  def Terminates(self):
    ...
    terminates = False
    pts = self._pycram_bullet_client.getContactPoints()
    for p in pts:
      part = -1
  ...
  def BuildHeadingTrans(self, rootOrn):
    ...
    eul = self._pycram_bullet_client.getEulerFromQuaternion(rootOrn)
    refDir = [1, 0, 0]
    rotVec = self._pycram_bullet_client.rotateVector(rootOrn, refDir)
    heading = math.atan2(-rotVec[2], rotVec[0])
    heading2 = eul[1]
    #print("heading=",heading)
    headingOrn = self._pycram_bullet_client.getQuaternionFromAxisAngle([0, 1, 0], -heading)
    return headingOrn
  ...
  def BuildOriginTrans(self):
    rootPos, rootOrn = self._pycram_bullet_client.getBasePositionAndOrientation(self._humanoid)

    #print("rootPos=",rootPos, " rootOrn=",rootOrn)
    invRootPos = [-rootPos[0], 0, -rootPos[2]]
    #invOrigTransPos, invOrigTransOrn = self._pycram_bullet_client.invertTransform(rootPos,rootOrn)
    headingOrn = self.BuildHeadingTrans(rootOrn)
    #print("headingOrn=",headingOrn)
    headingMat = self._pycram_bullet_client.getMatrixFromQuaternion(headingOrn)
    #print("headingMat=",headingMat)
    #dummy, rootOrnWithoutHeading = self._pycram_bullet_client.multiplyTransforms([0,0,0],headingOrn, [0,0,0], rootOrn)
    #dummy, invOrigTransOrn = self._pycram_bullet_client.multiplyTransforms([0,0,0],rootOrnWithoutHeading, invOrigTransPos, invOrigTransOrn)

    invOrigTransPos, invOrigTransOrn = self._pycram_bullet_client.multiplyTransforms([0, 0, 0],
                                                                                headingOrn,
                                                                                invRootPos,
                                                                                [0, 0, 0, 1])
    #print("invOrigTransPos=",invOrigTransPos)
    #print("invOrigTransOrn=",invOrigTransOrn)
    invOrigTransMat = self._pycram_bullet_client.getMatrixFromQuaternion(invOrigTransOrn)
  ...
  def ApplyAction(self, action):
    ...
    index += 4
    pose._chestRot = self._pycram_bullet_client.getQuaternionFromAxisAngle(axis, angle)
    #print("pose._chestRot=",pose._chestRot)

    angle = action[index]
    axis = [action[index + 1], action[index + 2], action[index + 3]]
    index += 4
    pose._neckRot = self._pycram_bullet_client.getQuaternionFromAxisAngle(axis, angle)

    angle = action[index]
    axis = [action[index + 1], action[index + 2], action[index + 3]]
    index += 4
    pose._rightHipRot = self._pycram_bullet_client.getQuaternionFromAxisAngle(axis, angle)
    ...
    axis = [action[index + 1], action[index + 2], action[index + 3]]
    index += 4
    pose._rightAnkleRot = self._pycram_bullet_client.getQuaternionFromAxisAngle(axis, angle)

    angle = action[index]
    axis = [action[index + 1], action[index + 2], action[index + 3]]
    index += 4
    pose._rightShoulderRot = self._pycram_bullet_client.getQuaternionFromAxisAngle(axis, angle)
    ...
    axis = [action[index + 1], action[index + 2], action[index + 3]]
    index += 4
    pose._leftHipRot = self._pycram_bullet_client.getQuaternionFromAxisAngle(axis, angle)
    ...
    axis = [action[index + 1], action[index + 2], action[index + 3]]
    index += 4
    pose._leftAnkleRot = self._pycram_bullet_client.getQuaternionFromAxisAngle(axis, angle)

    angle = action[index]
    axis = [action[index + 1], action[index + 2], action[index + 3]]
    index += 4
    pose._leftShoulderRot = self._pycram_bullet_client.getQuaternionFromAxisAngle(axis, angle)
  ...
  def GetState(self):
    ...
    rootPosRel, dummy = self._pycram_bullet_client.multiplyTransforms(rootTransPos, rootTransOrn,
                                                                 basePos, [0, 0, 0, 1])
    #print("!!!rootPosRel =",rootPosRel )
    #print("rootTransPos=",rootTransPos)
    #print("basePos=",basePos)
    localPos, localOrn = self._pycram_bullet_client.multiplyTransforms(rootTransPos, rootTransOrn,
                                                                  basePos, baseOrn)
    ...
    for pbJoint in range(self._pycram_bullet_client.getNumJoints(self._humanoid)):
      ...
      #print("joint order:",j)
      ls = self._pycram_bullet_client.getLinkState(self._humanoid, j, computeForwardKinematics=True)
      linkPos = ls[0]
      linkOrn = ls[1]
      linkPosLocal, linkOrnLocal = self._pycram_bullet_client.multiplyTransforms(
    ...
    for pbJoint in range(self._pycram_bullet_client.getNumJoints(self._humanoid)):
      j = self.pb2dmJoints[pbJoint]
      ls = self._pycram_bullet_client.getLinkState(self._humanoid, j, computeLinkVelocity=True)
      linkLinVel = ls[6]
      linkAngVel = ls[7]
      for l in linkLinVel:
        stateVector.append(l)
  ...
  def GetReward(self):
    ...
    for j in range(num_joints):
      ...
      simJointInfo = self._pycram_bullet_client.getJointStateMultiDof(self._humanoid, j)
      ...
      if (len(simJointInfo[0]) == 4):
        ...
        diffQuat = self._pycram_bullet_client.getDifferenceQuaternion(simJointInfo[0], kinJointInfo[0])
        axis, angle = self._pycram_bullet_client.getAxisAngleFromQuaternion(diffQuat)
        curr_pose_err = angle * angle
        diffVel = [
