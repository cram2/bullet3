...
class BulletSim():
  ...
  def __enter__(self):
    ...
    cid = pycram_bullet.connect(self.connection_mode, options=optionstring, *self.argv)
    pycram_bullet.setAdditionalSearchPath(pycram_bullet_data.getDataPath())
    if cid < 0:
      raise ValueError
    print("connected")
    pycram_bullet.configureDebugVisualizer(pycram_bullet.COV_ENABLE_GUI, 0)
    pycram_bullet.configureDebugVisualizer(pycram_bullet.COV_ENABLE_SEGMENTATION_MARK_PREVIEW, 0)
    pycram_bullet.configureDebugVisualizer(pycram_bullet.COV_ENABLE_DEPTH_BUFFER_PREVIEW, 0)
    pycram_bullet.configureDebugVisualizer(pycram_bullet.COV_ENABLE_RGB_BUFFER_PREVIEW, 0)

    pycram_bullet.resetSimulation()
    pycram_bullet.loadURDF("plane.urdf", [0, 0, -1])
    pycram_bullet.loadURDF("r2d2.urdf")
    pycram_bullet.loadURDF("duck_vhacd.urdf")
...
def test(num_runs=300, shadow=1, log=True, plot=False):
  if log:
    logId = pycram_bullet.startStateLogging(pycram_bullet.STATE_LOGGING_PROFILE_TIMINGS, "renderTimings")
  ...
  for i, yaw in zip(range(num_runs), yaw_gen):
    ...
    #renderer=pycram_bullet.ER_TINY_RENDERER)
    stop = time.time()
    duration = (stop - start)
    if (duration):
      fps = 1. / duration
  ...
  print("")
  if log:
    pycram_bullet.stopStateLogging(logId)
  return mean_time
...
if __name__ == "__main__":
  ...
  with BulletSim(pycram_bullet.DIRECT):
    print("\nTesting DIRECT")
  ...
  with BulletSim(pycram_bullet.DIRECT):
    plugin_fn = os.path.join(
        pycram_bullet.__file__.split("bullet3")[0],
        "bullet3/build/lib.linux-x86_64-3.5/eglRenderer.cpython-35m-x86_64-linux-gnu.so")
    plugin = pycram_bullet.loadPlugin(plugin_fn, "_tinyRendererPlugin")
    if plugin < 0:
  ...
  with BulletSim(pycram_bullet.GUI):
    print("\nTesting GUI")
