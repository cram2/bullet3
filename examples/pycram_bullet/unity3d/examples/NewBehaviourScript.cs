...
public class NewBehaviourScript : MonoBehaviour {
	...
	void Start () {
        ...
        pycram_bullet = NativeMethods.b3ConnectSharedMemory(NativeConstants.SHARED_MEMORY_KEY);
        if (NativeMethods.b3CanSubmitCommand(pycram_bullet)==0)
        {
        ...
        IntPtr cmd = NativeMethods.b3InitResetSimulationCommand(pycram_bullet);
        IntPtr status = NativeMethods.b3SubmitClientCommandAndWaitStatus(pycram_bullet, cmd);

        {
            IntPtr command = NativeMethods.b3InitPhysicsParamCommand(pycram_bullet);
            NativeMethods.b3PhysicsParamSetGravity(command, 0, -10, 0);
            IntPtr statusHandle = NativeMethods.b3SubmitClientCommandAndWaitStatus(pycram_bullet, command);
        }

        int numBodies = NativeMethods.b3GetNumBodies(pycram_bullet);
        {
            cmd = NativeMethods.b3LoadUrdfCommandInit(pycram_bullet, "plane.urdf");
            Quaternion qq = Quaternion.Euler(-90, 0, 0);
            NativeMethods.b3LoadUrdfCommandSetStartOrientation(cmd, qq.x, qq.y, qq.z, qq.w);
            status = NativeMethods.b3SubmitClientCommandAndWaitStatus(pycram_bullet, cmd);
        }

        
        cmd = NativeMethods.b3LoadUrdfCommandInit(pycram_bullet, "cube.urdf");
        ...
        {
            numBodies = NativeMethods.b3GetNumBodies(pycram_bullet);
            text.text = numBodies.ToString();
            cmd = NativeMethods.b3InitRequestVisualShapeInformation(pycram_bullet, m_cubeUid);
            status = NativeMethods.b3SubmitClientCommandAndWaitStatus(pycram_bullet, cmd);
            ...
            if (statusType == (EnumSharedMemoryServerStatus)EnumSharedMemoryServerStatus.CMD_VISUAL_SHAPE_INFO_COMPLETED)
            {
                ...
                NativeMethods.b3GetVisualShapeInformation(pycram_bullet, ref visuals);
            ...
            if (numBodies > 0)
            {
                ...
                NativeMethods.b3GetBodyInfo(pycram_bullet, 0, ref  info );
    ...
    void Update () {
        if (pycram_bullet != IntPtr.Zero)
        {
            IntPtr cmd = NativeMethods.b3InitStepSimulationCommand(pycram_bullet);
            IntPtr status = NativeMethods.b3SubmitClientCommandAndWaitStatus(pycram_bullet, cmd);
        }
        if (m_cubeUid>=0)
        {
            IntPtr cmd_handle =
                NativeMethods.b3RequestActualStateCommandInit(pycram_bullet, m_cubeUid);
            IntPtr status_handle = NativeMethods.b3SubmitClientCommandAndWaitStatus(pycram_bullet, cmd_handle);
    ...
    void OnDestroy()
    {
        if (pycram_bullet != IntPtr.Zero)
        {
            NativeMethods.b3DisconnectSharedMemory(pycram_bullet);
        }
